...
_last_speak_ts = 0.0
_last_alert_ts = 0.0
MIN_SPEAK_GAP_FRAME = 3
MIN_ALERT_GAP_FRAME = 3
per_target_cooldown = {}
TARGET_COOLDOWN_FRAME = 15
...
def speak(txt, frame_idx, priority=False):
    global _last_speak_ts, _last_alert_ts
    now = frame_idx
    if priority:
        if now - _last_alert_ts < MIN_ALERT_GAP_FRAME: return
        with _tts_lock:
            try:
        ...
        if now - _last_speak_ts < MIN_SPEAK_GAP_FRAME: return
        _last_speak_ts = now
    try:
...
while True:
    ...
    if alert_text:
        ...
        if (direction != last_direction):
            ...
            speak(fala, frame_idx, priority=True)
            last_direction = direction
    elif last_direction is not None:
        fala = "Caminho livre"
        speak(fala, frame_idx)
        last_direction = None
        continue_time = frame_idx
    if continue_time is not None and frame_idx - continue_time > 60:
        fala = "Livre"
        speak(fala, frame_idx)
        continue_time = frame_idx
